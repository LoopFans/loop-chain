...
HTTPS_GIT := github.com/LoopFans/loop-chain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=loopchain \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/loopd ./cmd/loopd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/loopd.exe ./cmd/loopd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/loopd
...
draw-deps:
	...
	@goviz -i ./cmd/loopd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c loopchain --local -f chains.yaml
endif
...
set-testnet-configs:
	loopd config set client chain-id chainid-1
	loopd config set client keyring-backend test
	loopd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | loopd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | loopd keys add acc1 --recover`
