...
# then log output of file /code/bin/loopd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/loopd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/loopd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/loopd /usr/bin/loopd
